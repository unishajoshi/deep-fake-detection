...
def cosine_similarity(vec1, vec2):
    vec1 = np.array(vec1)
    vec2 = np.array(vec2)
    return dot(vec1, vec2) / (norm(vec1) * norm(vec2) + 1e-6)
...
def analyze_face(image_path):
    ...
    return {
        ...
        'embedding': embedding.tolist()  # stored as list for later JSON/CSV use
    }
...
def match_best_video_by_face_attributes(image_path, frame_df, frame_index, frame_dir="all_data_frames"):
    ...
    for _, row in candidate_rows.iterrows():
        ...
        def compute_score(frame_row):
            ...
            try:
                frame_embedding = eval(frame_row['embedding']) if isinstance(frame_row['embedding'], str) else frame_row['embedding']
                sim_score = cosine_similarity(img_attrs["embedding"], frame_embedding)
            except Exception as e:
                print(f"[WARN] Failed cosine similarity: {e}")
                sim_score = 0  # fallback
        
            total_score = (
                3.0 * sim_score +            # highest weight for facial similarity
...
def generate_synthetic_videos(image_df, streamlit_progress=None, st_module=None):
    ...
    frame_attrs_df["video_id"] = frame_attrs_df["frame_file"].str.extract(r"(.*)_frame\d+")
